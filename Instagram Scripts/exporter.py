...
Usage:
    python export_highlights.py                  # GUI
    python export_highlights.py --all             # export all, no GUI
    python export_highlights.py --all --no-counter   # hide "n/n" (shown by default)
    python export_highlights.py --all --no-reply-bar
    python export_highlights.py --all --mute       # drop original video audio
"""

import tkinter as tk
from tkinter import ttk, messagebox
import threading, subprocess, json, os, sys, shutil, tempfile, argparse, math, io, urllib.request
...
SUBTEXT    = (220, 220, 220)
CIRC_COL = (245, 245, 245) #(193, 53, 132)
...
_EMOJI_FLAG_RE = re.compile(r'([\U0001f1e6-\U0001f1ff]{2})\s*$')
_TEXT_FLAG_RE = re.compile(r'(?:\s|^)\[?([A-Za-z]{2})\]?\s*$')


def extract_country_code(title: str) -> tuple[str, str | None]:
    """Match the title/flag convention used by viewer.py.

    A collection title can end in a flag emoji (``Portugal 🇵🇹``) or a country
    code (``Portugal PT`` / ``Portugal [PT]``).  The returned title excludes
    that suffix so the code is rendered as the matching flag image instead.
    """
    m_emoji = _EMOJI_FLAG_RE.search(title)
    if m_emoji:
        cc = "".join(chr(ord(char) - 127397) for char in m_emoji.group(1))
        return title[:m_emoji.start()].rstrip(), cc

    m_text = _TEXT_FLAG_RE.search(title)
    if m_text:
        return title[:m_text.start()].rstrip(), m_text.group(1).upper()

    return title, None


def clean_title(title: str) -> str:
    """Backward-compatible alias for callers that only need the visible title."""
    return extract_country_code(title)[0]


def _fetch_flag_image(country_code: str, height: int = 26) -> Image.Image | None:
    """Load the country flag locally, downloading and caching it if needed.

    This is intentionally the same source order and on-disk cache as viewer.py,
    so flags look consistent in the preview and exported MP4.
    """
...
def _emoji_clusters(text: str) -> list[str]:
    """Keep variation selectors and ZWJ modifiers attached to their emoji."""
    clusters: list[str] = []
    current = ""
    for char in text:
        is_modifier = char in ("\ufe0e", "\ufe0f", "\u200d") or "\U0001f3fb" <= char <= "\U0001f3ff"
        if current and (is_modifier or current.endswith("\u200d")):
            current += char
        else:
            if current:
...
def _draw_compact_emoji_title(d, xy, clusters, font, fill):
    """Draw an emoji-only title with a fixed natural emoji cell width.

    Segoe UI Emoji reports a 94 px advance for e.g. ☠️ at 34 px, leaving a
    conspicuous gap before the following emoji. Its visible glyph is much
    narrower, so a 48 px cell produces the expected adjacent appearance.
    """
    x, y = xy
    for cluster in clusters:
        _text_shadow(d, (x, y), cluster, font, fill)
        x += 48
    return x - xy[0]
...
def build_overlay_frame(n_items: int, current_idx: int, title: str,
    ...
    clean, country_code = extract_country_code(title)
    flag = _fetch_flag_image(country_code) if country_code else None
    flag_gap = 16 if flag is not None else 0
    flag_w = flag.width + flag_gap if flag is not None else 0
    max_text_w = more_cx - 18 - TEXT_X
    badge_w = 30 if verified else 0

    fn_title = _font(34, "medium")
    emoji_clusters = []
    # Pillow nie dobiera automatycznie fontu emoji.
    # Dla tytułów złożonych wyłącznie z emoji używamy fontu systemowego.
    if clean and not any(char.isalnum() for char in clean):
        try:
            fn_title = ImageFont.truetype(
                r"C:\Windows\Fonts\seguiemj.ttf", 34
            )
            emoji_clusters = _emoji_clusters(clean)
        except OSError:
            pass
    display = clean if emoji_clusters else _fit_title(
        d, clean, fn_title, max_text_w - badge_w - flag_w
    )
    title_width = 48 * len(emoji_clusters) if emoji_clusters else d.textlength(display, font=fn_title)
    ...
    if show_counter:
        ...
        if emoji_clusters:
            _draw_compact_emoji_title(d, (TEXT_X, ty), emoji_clusters, fn_title, (*TEXT_COL, 245))
        else:
    ...
    if verified:
        ...
        _draw_verified_badge(d, TEXT_X + title_width + 18, int(title_center_y))

    if flag is not None:
        # Align the cached PNG with the title baseline, just as viewer.py puts
        # its 18 px flag beside the collection title in the story header.
        flag_y = int(ty + tb[1] + (title_h - flag.height) / 2)
        overlay.paste(flag, (int(TEXT_X + title_width + 12 + badge_w), flag_y), flag)
...
class ExporterApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.photo_sec_var = tk.StringVar(value="3")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--photo-sec", type=float, default=3.0,
