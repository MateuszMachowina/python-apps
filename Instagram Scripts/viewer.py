...
# ─── Title sanitizing (mirrors downloader.py's fix) ──────────────────────────────
def _fix_title_unicode(title: str) -> str:
    """
    Defensive safety net matching downloader.py's fix of the same name: if an
    older meta.json somehow has a title with an unpaired UTF-16 surrogate
    (a known Instagram API quirk around emoji, including flags), repair or
    drop it instead of ever raising while rendering the sidebar/header.
    """
    if not title:
        return title
    try:
...
def extract_country_code(title: str) -> tuple[str, str | None]:
    ...
    if m_emoji:
        cc = "".join(chr(ord(c) - 127397) for c in m_emoji.group(1))
...
def load_collections() -> list[dict]:
    ...
    if not HIGHLIGHTS_DIR.exists():
        return cols
    for d in HIGHLIGHTS_DIR.iterdir():
        ...
        meta["title"] = _fix_title_unicode(meta.get("title", d.name))
        meta["_dir"] = d
        meta["_files"] = [
            d / it["file"]
        ...
        if meta["_files"]:
            cols.append(meta)
    cols.sort(key=lambda x: x["_downloaded_at"], reverse=False)
    return cols
...
class ViewerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.title_flag_lbl = tk.Label(title_row, bg=BG)
        self.title_flag_lbl.pack(side="left", padx=(8, 0))
        ...
        self.bind("<Up>",     lambda e: self._prev_collection())
        self.bind("<Down>",   lambda e: self._next_collection())
    ...
    def _update_story_header(self):
        ...
        self.title_lbl.config(text=clean)
        
        if cc:
            import threading
            def _load_main_flag():
                img = _fetch_flag_image(cc, height=18) 
                if img:
                    ph = ImageTk.PhotoImage(img)
                    self.title_flag_lbl.after(0, lambda: self._apply_main_flag(ph))
            threading.Thread(target=_load_main_flag, daemon=True).start()
        else:
            self.title_flag_lbl.config(image="")
    ...
    def _apply_main_flag(self, ph):
        self.title_flag_lbl.config(image=ph)
        self.title_flag_lbl.image = ph
    ...
    def _highlight_sidebar(self, idx: int):
        ...
        try:
            total = max(1, len(self.sb_inner.winfo_children()))
            self.sb_canvas.yview_moveto(idx / total)
        except Exception:
            pass
    ...
    def _next_collection(self):
        if not self.collections:
            return

        if self.active_coll is None:
            self._select_collection(0)
            return

        if self.active_coll < len(self.collections) - 1:
            self._select_collection(self.active_coll + 1)


    def _prev_collection(self):
        if not self.collections:
            return
